gdp_baseline = readtable('baseline_gdp_results.xlsx', 'Sheet', 'China');
gdp_scenario = readtable('gdp_results.xlsx', 'Sheet', 'China');
...
gdps = gdp_scenario.Growth_rate(4:56);
years = 1998:2050;
y_limits = [0, 0.12];
y_ticks = 0:0.02:0.12;
figure;
...
hold on;
plot(years(1:53), gdps, 'Color', colour2, 'LineStyle', '-.', 'LineWidth', 2);
ylim(y_limits);
yticks(y_ticks);
title('Real GDP Growth Rate of China', 'FontName', 'SansSerif');
legend('Baseline', 'TFP growth slowdown', 'Location', 'northeast', 'FontName', 'SansSerif');
