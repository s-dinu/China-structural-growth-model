...
objective_function <- function(params, data) {
  ...
  # alpha[25] <- data$relative_tfp[25]^(1/(1-theta))/gamma
  for (t in 2:106) {
    if (t == 2) {
  ...
  # penalty <- (alpha[25] - data$relative_tfp[25]^(1/(1-theta))/gamma)^2
  # total_objective <- sum_squared_diff + penalty
...
# Input table
input_path <- "demo_2_input.xlsx"
write.xlsx(usa_chn_data_1995, input_path, sheetName = "Sheet1", rowNames = FALSE)

# Exogenous variables
years <- 1995:2100
oecd_data <- read.xlsx("oecd_projections.xlsx", sheet = "EMPPOP Dec 2023")
projections <- data.frame(Year = 1995:2060, emppop_projections = oecd_data$EMPPOP_CHN)
projections_us <- data.frame(Year = 1995:2060, emppop_projections_us = oecd_data$EMPPOP_USA)
projections_path <- "demo_2_projections.xlsx"
write.xlsx(projections, projections_path, sheetName = "Sheet1", rowNames = FALSE)
projections_us_path <- "demo_2_projections_us.xlsx"
write.xlsx(projections_us, projections_us_path, sheetName = "Sheet1", rowNames = FALSE)
emp_pop_values <- c(
  projections$emppop_projections[projections$Year %in% 1995:2060],
  rep(projections$emppop_projections[projections$Year == 2060], times = 40)
)
emp_pop_values_us <- c(
  projections_us$emppop_projections_us[projections_us$Year %in% 1995:2060],
  rep(projections_us$emppop_projections_us[projections_us$Year == 2060], times = 40)
)
