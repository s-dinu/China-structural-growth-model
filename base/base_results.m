...
input = readtable('base_input.xlsx');
...
ylim(r_k_limits);
yticks(r_k_ticks);
title('Return to capital in China (%)', 'FontName', 'SansSerif');
legend('Model', 'Data: PWT', 'Data: corrected', 'Location', 'southeast', 'FontName', 'SansSerif');
source_text = 'Source: PWT 10.01, W. Chen et al. (2019), authors’ own calculation';
annotation('textbox', [0.115, 0, 0.8, 0.05], 'String', source_text, 'FontSize', 8, 'HorizontalAlignment', 'left', 'LineStyle', 'none', 'FontName', 'SansSerif');
set(gca, 'FontName', 'SansSerif');
...
writetable(gdp_chn_table, 'base_gdp_results.xlsx', 'Sheet', 'China');

% Extract TFP 
tfp_chn_table = table((1995:2100)', tfp(2:107), 'VariableNames', {'Year', 'TFP_baseline'});
writetable(tfp_chn_table, 'base_tfp.xlsx', 'Sheet', 'TFP');

% Extract y
y_chn_table = table((1995:2100)', y(2:107), 'VariableNames', {'Year', 'y_baseline'});
writetable(y_chn_table, 'base_y.xlsx', 'Sheet', 'y');