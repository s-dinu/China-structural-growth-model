...
unpop_data <- read.csv("unpop.csv")
filtered_unpop <- unpop_data %>%
  filter(Estimate_Type %in% c("CHN")) %>%
  select(Estimate_Method, Sex, Subregion) %>%
  rename(Year = Estimate_Method, Fertility_scenario = Sex, Value = Subregion)
years <- data.frame(Year = 1995:2100)
merged_unpop <- merge(years, filtered_unpop, by = "Year", all.x = TRUE)
final_unpop <- merged_unpop %>%
  select(Year, Fertility_scenario, Value) %>%
  pivot_wider(names_from = Fertility_scenario, values_from = Value) %>%
  rename(Median = 'Median', High_fertility = 'High-fertility', Low_fertility = 'Low-fertility')
final_unpop$Low_fertility[final_unpop$Year <= 2021] <- final_unpop$Median[final_unpop$Year <= 2021]
final_unpop$High_fertility[final_unpop$Year <= 2021] <- final_unpop$Median[final_unpop$Year <= 2021]

# Read Excel file
excel_file <- "pwt1001.xlsx"
data <- read_excel(excel_file, sheet=3)
...
objective_function <- function(params, data) {
  ...
  sum_squared_diff <- sum((data$relative_tfp[1:25] - model_tfp[1:25])^2)
  return(sum_squared_diff)
}
initial_params <- c(0.08, 0.22)
result <- optim(par = initial_params, fn = objective_function, data = usa_chn_data_1995, method = "CG")
optimal_eta <- round(result$par[1], digits = 2)
optimal_gamma <- round(result$par[2], digits = 2)

# Calibration results
result_table <- data.frame(
  Parameter = c("Discount factor", "Capital share, China", "Capital share, U.S.", "Depreciation rate, China", "Depreciation rate, U.S.", "Technology growth rate, U.S.", "Initial technology, China", "Catch-up bound", "Catch-up rate"),
  Value = c(beta_1995, theta, theta_star_1995, delta_1995, delta_star_1995, g, gamma0, optimal_gamma, optimal_eta)
)

# Input table
input_path <- "demo_1_input.xlsx"
...
pop_growth_values_m <- final_unpop$Median
pop_growth_values_l <- final_unpop$Low_fertility
pop_growth_values_h <- final_unpop$High_fertility
...
exo_var <- data.frame(
  ...
  n = round(pop_growth_values_l, digits = 4), # Change index for alternative fertility scenario (l - low, m - median, h - high)
